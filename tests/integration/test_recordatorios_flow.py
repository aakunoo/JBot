...
async def test_recordatorio_completo(real_db):  # real_db = fixture con Mongo real
    ...
    chat_id = 77
    user_id = chat_id  # En este caso usamos el mismo ID para chat y user
    models.register_user(chat_id=chat_id, user_id=user_id, username="tester", apodo="tester")
    ...
    docs = obtener_recordatorios(user_id)
    ...
    # 5) Lista final vacía
    assert obtener_recordatorios(user_id) == []
